...
#注意：改成了CPU版本,dataset输出时关闭数据增强，务必记住
############################################### 
def get_pred_pointcloud(dataset_path,index,npoints):

    #外参
    MODEL_ROOT = r'D:\\Project_on_going\\pointnet2_pytorch_part_seg\\log\sem_seg\downsampled'
    MODEL_VERSION = 'rrs-rrns-rrb'
    dr.record_data('版本',MODEL_VERSION)
...
def modelling_stairs_params(pcd_list,depthscale = 2500):
    stairs_params = []#第一个是height，第二个是width
    stairs = []
    def numpy_flat(a):
...
if __name__ == '__main__':
    ...
    #data recorder init
    experiment = 'normal'
    data_record_path = 'D:\Project_on_going\pointnet2_pytorch_part_seg\experiment'
    dr = data_recorder(experiment_name=experiment,write_path=data_record_path)

    #dataset path
    root = 'D:\Project_on_going\pointnet2_pytorch_part_seg\dataset'
    datasetpath = 'dataset_withlabell_eval_downsampled'
    ...
    for index in range(1,60):
        ...
        dr.record_data('日期',current_date)   
        #print("----------任务开始----------")
        dr.record_data('数据集',datasetpath)
        dr.record_data('index',index)
            
        #print("----------清晰楼梯识别开始----------")
        _, pred_choice, points_raw, _ = get_pred_pointcloud(rootpath,index,2500)
        ...
        #print("----------重建开始----------")
        pcd_meanpoints_fragments = get_3dreconstruct_and_meanpoints(pcd_fitted,pcd_raw,plane_params)
        modelling_stairs_params(pcd_meanpoints_fragments)
        #print("----------重建结束----------")

        #print("----------评估开始----------")
        dr.record_data('预测法向量',plane_params[0][:3])
        eval_norm_root =r'D:\Project_on_going\StairsSet\SZTUstairs\dataset_withlabell_eval_downsampled_stepnormal'
        normal_gt = read_GT_data(eval_norm_root,index)
        dr.record_data('GT法向量',normal_gt[0])
        error_norm = evaluate_position_direction(normal_gt[0],plane_params[0][:3])
        dr.record_data('余弦距离误差',error_norm)
        #print("----------评估结束----------")
