...
class Local_Planner():
    def __init__(self):
        ...
        # self.is_close = False # 是否到达目标点
        # self.is_get = False # 是否收到全局路径
        # self.is_grasp = False 
        self.is_all_task_down = False # 是否完成所有任务
        self.robot_state_set = False # 是否收到机器人状态
        self.ref_path_set = False  # 是否收到全局路径
        self.ob=[] # 障碍物
        self.is_end=0 # 是否到达终点
        self.ob_total = [] 
        self.__timer_replan = rospy.Timer(rospy.Duration(self.replan_period), self.__replan_cb) # 定时器，每隔0.01s执行一次replan_cb
        self.__sub_curr_state = rospy.Subscriber('/curr_state', Float32MultiArray, self.__curr_pose_cb, queue_size=10)
        self.__sub_obs = rospy.Subscriber('/obs', Float32MultiArray, self.__obs_cb, queue_size=10)  # 订阅障碍物
    ...
    def __replan_cb(self, event):
        if self.robot_state_set and self.ref_path_set:
            target = []
            self.choose_goal_state()        ##  global planning
            dist = 1
